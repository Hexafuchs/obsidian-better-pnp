import { App, Component, editorLivePreviewField, MarkdownView, MetadataCache, TFile } from 'obsidian';
...
export default function viewInlinePlugin(app: App, settings: BPSettings) {
    return ViewPlugin.fromClass(class implements PluginValue {
        ...
        addDeco(node: SyntaxNode, view: EditorView) {
            ...
            this.decorations.between(node.from - 1, node.to + 1, () => { exists = true });
            if (exists) {
